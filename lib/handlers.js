  ...
  if(firstName && lastName && phone && password && tosAgreement){
    ...
    _data.read('users',phone,function(err,data){
      if(err){
        ...
        // User alread exists
        callback(402,{'Error' : 'A user with that phone number already exists'});
      }
    });
...
handlers._checks.post = function(data,callback){
  ...
  var successCodes = typeof(data.payload.successCodes) == 'object' && data.payload.successCodes instanceof Array && data.payload.successCodes > 0 ? data.payload.successCodes : false;
